...
const MAXFRADirectorMVP = () => {
  ...
  const generateReceipt = () => {
    const customer = isNewReceiptStudent ? newReceiptStudent : selectedStudent;
    if (!customer || receiptItems.length === 0) return;
    ...
    ctx.textAlign = 'left';
    ctx.font = '11px Arial';
    ctx.fillText(`Cliente: ${customer.name}`, 10, y);
    y += 15;
    ctx.fillText(`Teléfono: ${customer.phone}`, 10, y);
    y += 15;
    if (customer.program) {
      ctx.fillText(`Programa: ${customer.program}`, 10, y);
      y += 20;
    } else {
      y += 5;
    }
    ...
    if (navigator.share && navigator.canShare) {
      canvas.toBlob(blob => {
        ...
        navigator.share({
          title: 'Recibo MAXFRA',
          text: `Recibo de pago - ${customer.name}`,
          files: [file]
        });
      });
    } else {
  ...
  // Calendar View
  const CalendarView = () => {
    const appointmentsInRange = getAppointmentsInRange(calendarDays, calendarStartDate);
    const dateRange = getDateRange(calendarDays, calendarStartDate);
                ...
                <button
                  onClick={() => {
                    if (newReceiptStudent.name && newReceiptStudent.phone) {
                      setSelectedStudent(newReceiptStudent);
                    } else {
